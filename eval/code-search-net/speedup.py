...
def speed_compare(args):
    ...
    data  = datasets.load_dataset('code_search_net', 'python', split='test')
    # breakpoint()
    ...
    for i in tqdm(data_lst): 
        d = data[i]
        processed_prompt = f"{roles[0]}: " + f"Please generate code based on the following doc:\n{d['func_documentation_string']}\n{roles[1]}: "
        #processed_prompt = prompt_mapping.format(input=d['func_documentation_string'])
...
if __name__ == "__main__":  
    ...
    parser.add_argument("--filename", type=str,
                        default="eval/code-search-net/test.jsonl")
