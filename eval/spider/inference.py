...
if __name__ == '__main__':
    ...
    if args.eval_only == False:
        ...
        for processed_prompt in tqdm(raw_data):
            input_ids = torch.tensor(processed_prompt['sources_input_ids']).long()[None]
            ...
            outputs.append({'gold': f'{processed_prompt["query"]}\t{processed_prompt["db_id"]}', 'pred': output})
        print('>>>>>> generation done')
        all_golds = [x['gold'] for x in outputs]
        all_preds = [x['pred'] for x in outputs]
        ...
        with open(args.output_file_name+'-gold.txt', "w") as f:
            f.write('\n'.join(all_golds))
        with open(args.output_file_name+'-pred.txt', "w") as f:
            f.write('\n'.join(all_golds))
