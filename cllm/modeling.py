...
class TimestepEmbeddingsForDiffusion(nn.Module):
    ...
    def forward(self, timestep):
        ...
        timesteps_emb = self.timestep_embedder(timesteps_proj.to(self.timestep_embedder.linear_1.weight.dtype))  # (N, D)

        conditioning = timesteps_emb

        return conditioning
    
class LLamaForMaskedDiffusion(LlamaForCausalLM):
    
    def __init__(self,config:LlamaConfig):
        super().__init__(config)
        if config.use_temporal_embedding:
            self.temporal_embedding = TimestepEmbeddingsForDiffusion(config.hidden_size)
